...
void
CheckForcedUnforcedDrops(QueueDisc::Stats st)
{ 
        
  std::cout << "*** ARED stats from Node 2 queue ***" << std::endl;
  std::cout << Simulator::Now().GetSeconds() << '\t'<< st << std::endl;
  Simulator::Schedule(MilliSeconds(500), &CheckForcedUnforcedDrops, st);
