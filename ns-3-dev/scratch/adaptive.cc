...
void
CheckForcedUnforcedDrops(QueueDiscContainer q)
{ 
        
   //std::cout << "Time\tUnforced\tForced" << std::endl;
   QueueDisc::Stats st = q.Get (0)->GetStats ();
   std::cout << Simulator::Now().GetSeconds() << '\t'<< st.GetNDroppedPackets (RedQueueDisc::UNFORCED_DROP)<<'\t'<<st.GetNDroppedPackets (RedQueueDisc::FORCED_DROP) << std::endl;
   Simulator::Schedule(MilliSeconds(50), &CheckForcedUnforcedDrops, q);
...
//Start editing
  ...
  Simulator::ScheduleNow (&CheckForcedUnforcedDrops, queueDiscs);
